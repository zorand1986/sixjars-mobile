...
const Setup = ({ navigation }) => {
  ...
  const renderItem = (item) => (
    <TouchableOpacity
      ...
      key={`${countries[item?.item]?.name}-${countries[item?.item]?.phone}`}
      style={[alignments.row,
    ...
    >
      ...
      <Text
        style={[basicText, alignments.alignCenter]}
      >
        {formatCurrency(countries[item?.item]?.currency || "USD")}
      </Text>
    </TouchableOpacity>
  );
